...
type TaskFilter struct {
	...
	// v0.29.0 (issue #81): + "created" — по дате создания, новые сверху;
	// дефолт (пусто) = "created" (раньше — "due").
	// "due": просроченные сверху, без срока внизу; v0.27.1 (хотфикс): без срока —
	// created_at DESC; id DESC — детерминированный ключ для пагинации.
	Sort string
	...
	// "none"     — план НЕ установлен (scheduled_date IS NULL; 28e — вкладка «План» → «Без плана»).
