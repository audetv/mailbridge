...
**Дата обновления:** 2026-09-16 (v0.24.0 выпущен; курсор → шаг 7, РЕЖИМ B; B-1 — фоновое наблюдение)
**Версия:** **v0.24.0 — ВЫПУЩЕН 2026-09-16** (тег `v0.24.0` → Release; блок «Персоны» = шаги 6 (✅) + 6-F (✅) + 6-G (✅)). Следующая минорная — **v0.25 = шаг 7 (Due date/SLA) + шаг 8**.
**Текущий шаг:** **шаг 7 (Due date/SLA) — РЕЖИМ B: обсуждение 5 решений владельца, код — только после их фиксации** (v0.25). v0.24.0 — ВЫПУЩЕН (тег `v0.24.0` → Release, 2026-09-16; блок «Персоны» 6 + 6-F + 6-G закрыт). **B-1** (наблюдение на проде 2–4 нед по метрикам confirmed) идёт **в фоне и НЕ блокирует ни один кодовый шаг**; **B-2** (правило авто-подтверждения) — отдельная мини-фича, только по данным B-1, не стоит в очереди перед шагом 7.
...
> **Порядок (ФИКСИРОВАН 2026-09-16, владелец):** 6-F ✅ → 6-G ✅ (PR #35) → тег v0.24.0 ✅ → **шаг 7 (v0.25, РЕЖИМ B: 5 решений)** → **шаг 8**. **B-1** — фоновое наблюдение на проде (2–4 нед), **не блокирует очередь**; **B-2** (правило авто-подтверждения) — только по данным B-1, отдельной мини-фичей (патч в v0.25 или отдельным шагом — решается по данным).
