...
**Дата обновления:** 2026-09-16 (обновлено: добавлен шаг 6-G; версия — на согласовании)
**Версия:** в разработке **v0.24 = блок «Персоны» = шаги 6 (✅) + 6-F (✅) + 6-G (новый)**. Следующая минорная — **v0.25 = шаг 7 (Due date/SLA)** + шаг 8. **v0.23.0 — выпущен 2026-09-15** (тег по решению владельца; блок 0–5 в `archive/PLAN.v0.23.md`).
**Текущий шаг:** **шаг 6-G (Ручное подтверждение персоны в UI) — РЕЖИМ A, старт после явного ОК владельца** (решение согласовано 2026-09-16; код — после ОК). Дальше = **шаг 7 (РЕЖИМ B, старт после 6-G)** — код только после фиксации 5 решений.
**Предыдущий шаг (завершён):** **шаг 6-F (hotfix чистота identity RFC822) — CLOSED 2026-09-16** (режим A; ParseFromHeader + EnsurePersonFromIncoming + эвристика «машина» + idempotent self-heal; квитанция в шаге 6-F; PR #33, squash `5bfa975`).
**Предыдущий (завершён):** **шаг 6 (Персоны) — CLOSED 2026-09-15** (режим A; PR #32, squash `eb9b3fc`).
...
### [ ] Шаг 6-G — Ручное подтверждение персоны в UI (v0.24, блок «Персоны») — РЕЖИМ A
**Добавлен 2026-09-16 по решению владельца (обсуждение после 6-F). Статус: решение зафиксировано — старт после явного ОК.**
...
**Почему здесь (до шага 7, в v0.24) — аргумента-дерева:**
...
**Что НЕ входит (границы):** авто-правила confirmation (B-2 — отдельно, по данным B-1); «отвергнуть»/match-rejections UI (задел шага 6, по опыту); RBAC. **Правило 10 соблюдены: отдельный номерной шаг, не «заодно» в шаге 7.**
...
**Квитанция (чек-лист закрытия):** CONTRIBUTING.md §Методология; PR `feat/6g-person-confirm` → CI green → merge (после явного ОК владельца на старт) → курсор → шаг 7.

### [ ] Шаг 7 — Due date: дата «до…/нужно до…» + SLA (v0.25) — РЕЖИМ B
...
### [ ] Шаг 8 — «Сегодня» / спринты: что отвечать сегодня (v0.25+) — РЕЖИМ B
...
## Открытые вопросы после 6-F (обсуждение 2026-09-16, решения владельца)

**Контекст:** после мержа 6-F (squash `5bfa975`) владелец: «вижу список не подтверждённых персон и никак не могу их подтвердить; только автоматическое создание, так?». Разбор по коду: **API уже умеет** — `PUT /api/persons/{id}` принимает `confirmed` (`internal/web/persons.go` UpdatePerson); **в UI нет кнопки/чекбокса «подтвердить»** (только read-only Tag «подтверждена/не подтверждена» в PersonsView.vue); **авто-путь на `confirmed=true` не существует** (backfill/AI/merge/suggest — все создают/оставляют `confirmed=false`).
...
> **Порядок (рекомендованный сценарий, на согласовании):** 6-F ✅ → **6-G (РЕЖИМ A)** → **тег v0.24.0 (стоп-гейт, владелец)** → **B-1 наблюдение** (2–4 нед.) → **B-2 (решение по данным)** → **шаг 7 (v0.25, РЕЖИМ B)** → шаг 8.
...
- **v0.24 (2026-09-15)** — в разработке (блок «Персоны»: шаги 6 + 6-F + 6-G; тег — стоп-гейт, после 6-G).
- **v0.25 (в разработке)** — шаг 7 (Due date/SLA, РЕЖИМ B) + шаг 8 (Сегодня/спринты); перенумерация с «v0.24 = 6–7» — рекомендованный сценарий, на согласовании.
