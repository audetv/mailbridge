...
1. **B-1 — Наблюдение на проде (наблюдательный пункт, НЕ кодовый).** После тега v0.24.0: 2–4 недели живых писем на новой сборке. Метрики: динамика `confirmed=false` (кол-во/темп), дубли (один email → N персон), no-reply vs живые. Сбор — SQL-срезы по `persons`/`person_identities` (квитанция 6-F). Владелец решает сроки остановки накопления.
2. **C-1 — Ручное подтверждение персоны в UI (новая фича, НЕ хотфикс).** API уже готов; не хватает «последней мили» — чекбокс/кнопка подтверждения в PersonsView (карточка/профиль) + e2e-тест. Оценка: ~30–50 строк Vue + 1 spec. **Место в плане: отдельный шаг ПОСЛЕ релиза v0.24.0** (не «заодно» в шаге 7 — Правило 10; не хотфикс — это фича).
3. **B-2 — Правило авто-подтверждения: ОТКРЫТО до B-1.** Кандидаты (обсуждение 2026-09-16): «ручное подтверждение закрывает вопрос (C-1)»; «N писем + стабильное имя»; «merge подтверждает целевую персону». **Решение — только по данным B-1. Пока открыт: не в код, не в шаги.**

> **Порядок (план v0.24.0+):** 6-F ✅ → **шаг 7 (Due/SLA, РЕЖИМ B)** → тег v0.24.0 (владелец, стоп-гейт) → **B-1 наблюдение** → **C-1 отдельный шаг** → B-2 (открытый вопрос к обсуждению).
