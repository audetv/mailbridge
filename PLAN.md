...
**Текущий шаг:** v0.24.0 ВЫПУЩЕН (тег `v0.24.0` → Release, 2026-09-16; блок «Персоны» 6 + 6-F + 6-G закрыт). Дальше = **B-1** (наблюдение на проде 2–4 нед по метрикам confirmed) → **B-2** (правило авто-подтверждения по данным) → **шаг 7 (РЕЖИМ B, старт после B-2)** — код только после фиксации 5 решений.
