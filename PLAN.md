...
**Дата обновления:** 2026-09-16 (v0.24.0 выпущен; шаг 7 РЕЖИМ B закрывается — решения зафиксированы; курсор → **7a**, РЕЖИМ A; B-1 — фоновое наблюдение)
...
**Текущий шаг:** **шаг 7 (Due date) — решения ЗАФИКСИРОВАНЫ 2026-09-16** (11 пунктов + разбивка 7a–7d; §шаг 7 ниже). **Далее — РЕЖИМ A, старт с подшага 7a** (ядро: поля срока + API) — каждый подшаг: 1 цель, 1 PR, 1 CI green, 1 merge, курсор на следующий (новая сессия). v0.24.0 — ВЫПУЩЕН (тег `v0.24.0` → Release, 2026-09-16; блок «Персоны» 6 + 6-F + 6-G закрыт). **B-1** (наблюдение на проде 2–4 нед по метрикам confirmed) идёт **в фоне и НЕ блокирует ни один кодовый шаг**; **B-2** (правило авто-подтверждения) — отдельная мини-фича, только по данным B-1, не стоит в очереди перед шагом 7.
...
### [ ] Шаг 7 — Due date: срок «до…/нужно до…» (v0.25) — РЕШЕНИЯ ЗАФИКСИРОВАНЫ 2026-09-16

**Цель.** Задача знает утверждённый срок («до 18.09») — расцветка/бейдж + сортировка; AI предлагает срок из письма, человек принимает/меняет/отклоняет. (Опора на шаг 6: `is_internal` на персонах заделан под будущее SLA.)

**Решения владельца (зафиксировано 2026-09-16, РЕЖИМ B закрыт — далее РЕЖИМ A):**
1. **Срок = дата без времени** (`due_date DATE`). Задача без срока — **норма**, ничего не алертится.
2. **AI извлекает явно обозначенные сроки** из письма («до 18.09», «нужно до…»); срока в письме нет → `null`. Промпт дополняется заданием; в вердикт (new **и** update) — `"due_date": "YYYY-MM-DD" | null`.
3. **AI предлагает, человек решает.** Предложение — `ai_due_date` + `due_ai_pending=true`; до принятия НЕ влияет на ничего (расцветка/алерты — только утверждённый срок). UI: подсветка строки + бейдж «срок? (AI)» + действия **принять / изменить / отклонить**.
4. **Ручное всегда приоритетнее.** `due_source='ai'` (принят из предложения) | `'manual'` (поставлен/изменён человеком) — **изменённое после AI = manual** (решение владельца 2026-09-16). Новые AI-предложения при manual-сроке — только в `ai_due_date` + pending, `due_date` не трогают.
5. **База ошибок AI:** `ai_due_date` сохраняется ВСЕГДА (включая отклонённые) — статистика «AI попал / упустил / заврался» для улучшения распознавания по данным (наследие B-1-методики).
6. **Флаг `MAILBRIDGE_AI_AUTO_APPLY_DUE`** (default **false** — ждать подтверждения; true = AI-сроки применяются сразу, source=`ai`). Заложен в v0.25, выключен; режим авто — по опыту.
7. **Визуально — только расцветка/бейдж** (просрочено=красный, сегодня=жёлтый, дальше=нейтральный) + сортировка; **напоминания/push/WS — НЕ делаем сейчас (зафиксировано отложить)**.
8. **SLA-нормативы — НЕ делаем сейчас (зафиксировано отложить):** нужен опыт и статистика; опора готова (`is_internal`, `due_date`, база ошибок AI).
9. **UI:** колонка «Срок» + бейдж; селектор сортировки **«по срокам / по активности»** (дефолт по вкладке); вкладка **«Все»** (все статусы, по умолчанию «по активности» — ответ на письмо/комментарий поднимает задачу наверх); фильтры по срокам: просроченные / сегодня / завтра / 7 дней / 30 дней / без срока / **срок на подтверждении** (pending) — серверные (через пагинацию).
10. **«Личное назначение» (своё на сегодня/неделю) = срок на нужную дату + фильтры по срокам** — отдельного поля НЕТ; «свой vs клиентский срок» — отложено (`due_origin`).
11. **Разбивка на подшаги (принята владельцем 2026-09-16):** 7a → 7b → 7c → 7d, каждый = 1 цель/1 PR/1 CI/1 merge — компактные шаги для приёмственности сессий (каждый следующий стартует в новой сессии с чистого состояния). Все 4 подшага входят в **v0.25**.

**Подшаги (реализация в этом порядке):**
- **[ ] 7a — ядро: поля срока + API.** Миграция `tasks`: `due_date DATE NULL`, `ai_due_date DATE NULL`, `due_source TEXT NULL CHECK (due_source IN ('ai','manual'))`, `due_ai_pending INTEGER NOT NULL DEFAULT 0` (idempotent). API: поля в GET списка/детали; постановка/смена/очистка ручного срока; серверная сортировка `?sort=due|updated` (due: просроченные сверху, без срока внизу; updated: по свежести). Unit-тесты (store + handler). **Приёмка:** срок ставится/изменяется/очищается через API, переживает рестарт; сортировка работает через пагинацию.
- **[ ] 7b — AI-предложение срока.** Промпт: явно обозначенные сроки; вердикт new/update: `due_date: "YYYY-MM-DD" | null`. Логика: AI пишет только `ai_due_date`+pending (кроме авто-режима флага); manual-срок НЕ перезаписывается; `ai_due_date` хранится всегда (база ошибок). Флаг `MAILBRIDGE_AI_AUTO_APPLY_DUE=false` в config + `config.example.env`. Unit-тесты (схема вердикта, приоритет manual, флаг). **Приёмка:** письмо «сделайте до 18.09» → pending-предложение; с manual-сроком AI не трогает; по умолчанию `false`.
- **[ ] 7c — подтверждение в UI.** Карточка задачи: поле даты (поставить/изменить/очистить, source=manual). Строка: бейдж «срок? (AI: дата)» + **принять / изменить (AI-значение предзаполнено) / отклонить**; pending-подсветка. Расцветка: просрочено=красный, сегодня=жёлтый, дальше=нейтральный, без срока=пусто; колонка «Срок». Unit (компоненты) + **e2e Playwright** (цикл принять/изменить/отклонить). **Приёмка:** полный цикл «AI предложил → человек решил» работает в UI; отклонённое остаётся в `ai_due_date`.
- **[ ] 7d — вкладка «Все» + режимы сортировки + фильтры по срокам.** Вкладка «Все» (все статусы, дефолт «по активности»). Селектор «по срокам / по активности» — одна контрольная, дефолт зависит от вкладки («Все»→активность; статусные→сроки). Фильтры: просроченные/сегодня/завтра/7д/30д/без срока/pending — серверные. **e2e Playwright** (обязательный триггер: вкладка/фильтры/сортировка). **Приёмка:** «кинул на сегодня» — наверху в «Все»; просроченные — наверху в статусных вкладках; задача с новым комментарием не тонет.

**Отложено (зафиксировано 2026-09-16, записано чтобы не потерять):** **SLA-нормативы** (нужен опыт и статистика; опора готова) · **напоминания/push/WS** (отдельная фича) · **`due_origin`** (свой vs клиентский срок) · **авто-принятие AI-сроков** (флаг есть с 7b, сейчас `false` — по опыту). · Шаг 8 («Сегодня»/спринты) остаётся отдельным; today/завтра-фильтры 7d — его фундамент.
