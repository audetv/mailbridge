...
| 13.3 | **E2E-infra (Playwright)**: `@playwright/test` в `frontend/`, `playwright.config.js` (baseURL :5173), fixture-seed в `data/` (script `frontend/scripts/seed-dev.mjs` через API); smoke `auth.spec.js` (login + вкладки видимы) | 13.2 | `[x]` (этот коммит) — config + auth.setup.js + seed-dev.mjs (идемпотентный) + auth.spec.js (smoke + deep-link) + npm e2e/e2e:seed; `npx playwright test` 3/3 green |
