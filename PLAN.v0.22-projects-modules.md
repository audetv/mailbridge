...
| 13.4 | **E2E-сценарии**: `projects-to-tasks.spec.js` — «Проекты → К задачам → «Активные» активна + filter project (все строки = проект) + селект-заполнен»; «фильтр проект/модуль в задаче-табах меняет список»; «создание из Проектов → задача с привязкой к проекту» (регрессия снятого бага 4); `npx playwright test` зелёный | 13.3 | `[x]` 967af40 — A–D spec: A «К задачам», B фильтр-проект, C фильтр-модуль (epic), D создание из Проектов; PrimeVue-хелперы (native mousedown/click, позиционные селекторы); `npx playwright test` 7/7 green 2 прогона |
