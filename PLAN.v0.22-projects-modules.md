...
| 18 | UI: в `TaskDetailView` две секции **Отчёт** и **Ответ пользователю** (вид `CommentList` по `kind`+`direction`); бейдж «Утверждён» на `reply`-комментарии с `approved=1`; стили по kind/author (агент `hermes` визуально выделяется); кнопка «**Закрыл задачу**» = **уже существующие** `WorkflowButtons` (new/in_progress → `closed`, in_progress → `completed` — не пишем новое); кнопка «**Утвердил ответ**» → `PATCH /api/comments/{id}/approve` (только на `kind=reply`, admin-only) | 17.1 | `[x]` 2026-08-30 — CommentList: бейджи kind (Отчёт/Ответ пользователю/Комментарий) + бейдж «Утверждён» + выделение агента; ReplyForm: селектор kind + кнопка «Утвердил ответ» (admin-only, kind=reply, approved≠1); tasks.js: approveComment + applyCommentApproved(event); websocket.js: onEvent (колбэки) + TaskDetailView: WS `comment_approved` → toast/мутация; **баг**: authStore.user терялся при F5 (user только в памяти) — restore() через GET /api/auth/me в main.js; vitest CommentList.spec.js 8/8 |
| 18.2 | Агент-юзер `hermes`: `auth.go` — 2 юзера: admin (`MAILBRIDGE_AUTH_USER/PASS`, дефолт admin/admin) + **hermes** (логин фиксированный `hermes`, `MAILBRIDGE_AGENT_PASS` обязательно для активации; юзер существует только если пароль задан); login отдаёт username; `extractUserFromToken` уже поддерживает (токен `token-<user>-<date>`); тесты auth (table-driven) | — | 8c6a5ec |
| 18.3 | API-разрешение: `POST /api/tasks/{id}/reply` принимает `kind: user_comment\|report\|reply` (default `user_comment`; валидация; любой аутентичный юзер может писать; `direction` = `out`) | 18.2 | `[x]` |
| 18.4 | **Фикс UI (фидбек админа 2026-08-30):** (а) кнопка «Утвердить» в `ReplyForm` (в `.buttons`) — **удалить**: дублирует per-comment кнопку, риск мисклика; approve — только под комментарием в `CommentList` (контекст: какой именно утверждается), wording → «Утвердить ответ» (инфинитив); (б) **баг**: Select `kind` без `optionValue` вёл в `v-model` ОБЪЕКТ `{value,label}` → бэкенд 400 `invalid request`, комментарий не создавался, ошибка в UI молча терялась (нет catch в `send()`) — починить: `optionValue="value"` + catch с сообщением об ошибке; (в) тесты: vitest-текст кнопки, E2E — отправка `kind=reply` **через UI** (селект) с проверкой бейджа | 18 | `[x]` — optionValue="value" + form-error (catch); approve-кнопка из ReplyForm удалена, «Утвердить ответ» (инфинитив) в CommentList; ReplyForm-тесты + E2E селект → 8/8 vitest, 11/11 E2E |
| 19 | `PATCH /api/comments/{id}/approve`: admin-only (не admin — 403), комментарий обязан быть `kind=reply` (иначе 400), `approved 0→1` (повторный approve идемпотентен); WS-событие `comment_approved` → UI бейдж в реальном времени | 18.3 | 8c6a5ec |
| 20 | **Приёмочный прогон** (кейс Лидер Спорт #66): dev `:8081` — агент (`hermes`) пишет `report` + `reply` к закрытой задаче → admin login в UI → видит оба комментария с бейджами → «Утвердил ответ» → бейдж появляется; WS-события приходят; approve от имени `hermes` → 403 | 19 | `[x]` 2026-08-30 (B: dev) — tests-e2e/approve.spec.js: UI approve (бейджи + кнопка + reload=1) + API guards (hermes→403, kind=report→400, повтор→200 idempotent); 4/4 green; весь E2E-набор 10/10 |
