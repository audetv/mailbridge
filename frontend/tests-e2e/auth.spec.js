...
test('dashboard after login: все 5 вкладок видимы (28d)', async ({ page }) => {
  ...
  for (const label of ['Лента', 'Активные', 'Сегодня', 'Проекты', 'Персоны']) {
    // 28e: вкладки-дропдауны (таб = пункт выбора): «Статус»→«Активные»,
    // «План»→«Сегодня» (дефолт-пункты; Bootstrap-паттерн)
    await expect(page.locator('.tab-bar button', { hasText: label }).first()).toBeVisible()
  }
})

test('deep-link: ?tab=status&status=active сразу на «Статус»/«Активные» (28e)', async ({ page }) => {
  await page.goto('/?tab=status&status=active')
  await expect(page).not.toHaveURL(/\/login/, { timeout: 10000 })

  // 28e: ОТДЕЛЬНОГО селекта нет — таб показывает выбранный пункт
  const statusTab = page.locator('button[data-testid="tab-status"]')
  await expect(statusTab).toHaveClass(/active/, { timeout: 10000 })
  await expect(statusTab.locator('.tab-dd-label')).toContainText('Активные', { timeout: 10000 })
})

test('deep-link: ?tab=plan&plan=today сразу на «План»/«Сегодня» (28e)', async ({ page }) => {
  await page.goto('/?tab=plan&plan=today')
  await expect(page).not.toHaveURL(/\/login/, { timeout: 10000 })

  const planTab = page.locator('button[data-testid="tab-plan"]')
  await expect(planTab).toHaveClass(/active/, { timeout: 10000 })
  const label = planTab.locator('.tab-dd-label')
  await expect(label).toContainText('Сегодня', { timeout: 10000 })
  // приёмка 28d/28e: deep-link переживает reload (URL — source of truth)
