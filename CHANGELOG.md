...
- **e2e-прогон детерминирован (issue #75, шаг 1, 2026-09-20):** `frontend/playwright.config.js` — `fullyParallel: false`, `workers: 1` (+ комментарий с механизмом). Корень флейка — НЕ баг отдельного теста: все e2e бьют в один общий живой бэкенд/SQLite/WS под одним логином «admin», и при `workers>1` WS-события воркера B падали на открытую страницу воркера A → `fetchTasks()`/`loading=true`/`.p-datatable-mask` накрывала его ассершны (жертва прыгает от прогона к прогону: bulk / «B: Проект»). Доказано: `workers=1` → 45/45 стабильно (4 прогона подряд), `workers=2` → падение в каждом из 3. Критерий закрытия: `npm run e2e` (дефолтная команда) — 4/4 прогона = 45/45 passed в dev-окружении. Истинная параллельная изоляция (per-worker бэкенд/БД) — BACKLOG-кандидат, не блокирует курс.

## [0.29.0] - 2026-09-20 (в разработке)

v0.29.0 = «По дате создания»: новый вариант сортировки и новый дефолт (issue #81, решения владельца 2026-09-20).

### Changed
- **Сортировка «По дате создания» — новый ДЕФОЛТ (шаг 29a, кандидат v0.29.0):** `GET /api/tasks` — `?sort=created` (`ORDER BY created_at DESC`, новые сверху) становится **дефолтом** (был `sort=due`); «По срокам» (`due`) и «По активности» (`updated`) остаются явными вариантами. Валидация `sort`: `""` / `due` / `updated` / `created` — ок (200), любое другое → **`400`** (как `?plan=`/`?due=`; молчаливый fallback скрывал бы typo-написание). UI: «По дате создания» — **первый пункт «Сортировки» и дефолт**; смена вкладки сбрасывает сортировку на «По дате создания». Тесты: Go `TestListTasks_ListSortCreatedDefault` + `TestListTasks_ListSortDueExplicit` (явный due, тик-брейкер v0.27.1 без срока = новые наверху сохранён) + `TestAPI_ListTasksSort` (RED→GREEN: 200 due/updated/created, 400 bogus/CREATED/asc); unit `tasks-tabs.spec.js` (сброс на created) + `DashboardTasksTab.spec.js` (mock); e2e `sort-created-29.spec.js` (3 теста: дефолт = новые сверху даже при просроченном сроке молодых, 400-on-junk, UI-первый пункт) + синка `due-filters-7d.spec.js` (7a-сценарий теперь с явным `?sort=due`). Доки: `docs/api.md` § query-параметры `sort`. **Прогон:** Go 76/76, vitest 184/184, golangci-lint 0 issues, eslint 0 warnings, e2e 48/48 (serial, workers:1).
...
- **Срез «План» `GET /api/tasks?plan=today|tomorrow|week` (v0.28, шаг 28b):** серверная логика — scheduled в окне (today = сегодня, tomorrow = завтра, week = 7 дней сегодня..+6) **+ в любом случае просроченные due** (`due_date < сегодня`, активные статусы `new`/`in_progress`/`backlog`; `completed`/`closed` — вне плана — решение владельца 2026-09-19: «совсем не видеть просроченные» отклонено). «Сегодня» — локальный день сервера (как `?due=`). Валидация `plan` → `400` на мусор. Тесты: `TestPlan_FilterByPlanBuckets` (все ветки: scheduled сегодня/неделя/вне, due просрочен в 3 активных статусах, excluded completed/closed, без дат, due=сегодня не просрочен, bogus не фильтрует) + `TestAPI_ListTasksPlan` (400 на `bogus`/`next`/`TODAY`, 200 на `today`/`tomorrow`/`week`, 200 без param). Доки: `docs/api.md` (§ query-параметры `plan`).
- **UI «План» (v0.28, шаг 28c):** детальная задача — поле «План» (DatePicker, установка → `PATCH {scheduled_date}`, снять → `null`; due не трогается — независимые поля); список задач — колонка «План» (тег с датой; тон другой, не «дедлайн»: план сегодня — `info`, дальше — `secondary`). Тесты: unit `TaskDetailScheduled.spec.js` (предзаполнение, PATCH scheduled_date, снятие, due не трогается) + `TaskTableScheduled.spec.js` (заголовки, теги, «—», тон info/secondary) + e2e `scheduled-date-28c.spec.js` (сценарии владельца: «due на неделю → scheduled сегодня клик-календарём», «due=сегодня → scheduled завтра, due остаётся как есть»). Доки: `docs/api.md`.
- **Переорганизация вкладок: «Статус» + «План» + селект (v0.28, шаг 28d):** 5 статусных вкладок (Все/Активные/Бэклог/Выполненные/Закрытые) → **вкладка «Статус» + селект** (Все/Активные/Бэклог/Выполненные/Закрытые = прежние наборы статусов) и **вкладка «План» + селект** (Сегодня/Завтра/Неделя = `?plan=today|tomorrow|week`, срез 28b). Порядок вкладок: **Лента → Статус → План → Проекты → Персоны** (решение владельца 2026-09-20). URL — source of truth: `?tab=<key>&status=<val>` (вкладка «Статус») / `?tab=plan&plan=<val>` (вкладка «План»); `?status=`/`?plan=` — только в своей вкладке, при смене вкладки другой параметр из URL убирается (старые `?tab=all|active|backlog|completed|closed` ломаются — миграции нет, решение владельца). Селект над списком задач виден только в своей вкладке; deep-link из Проектов/Персон/«Проект» в таблице задач теперь `?tab=status&status=active&…`. Тесты: RED→GREEN `tests/stores/tasks-tabs.spec.js` (стор `STATUS_FILTER`, `setStatusFilter`/`setPlanFilter`, fetch params) + `tests/views/DashboardTasksTab.spec.js` (5 вкладок, deep-link `?tab=status&status=active`/`?tab=plan&plan=today`, смена селекта, смена вкладки сбрасывает внешний параметр, fallback на «Статус» + «Все» при невалидном/старом `?tab=`/`?status=`/`?plan=`, «Создать задачу» в обеих вкладках); e2e — `auth.spec.js` (5 вкладок + both deep-links), `projects-to-tasks.spec.js`/`persons.spec.js`/`due-filters-7d.spec.js`/`websocket-reconnect.spec.js` переложены на новую схему. Доки: `docs/api.md` (§ UI).
- **Вкладки-дропдауны (v0.28, шаг 28e):** решение владельца 2026-09-20 (аналог Bootstrap «Tabs with dropdowns») — «Статус» и «План» стали **вкладками-дропдаунами**: таб — дропдаун-кнопка, клик открывает меню, выбор — вкладка активна и **таб показывает выбранный пункт**; отдельный селект-фильтр над списком **убран** (UI шага 28d откатился, API/URL-схема сохранена). Пункты: «Статус» — Все/Активные/Бэклог/Выполненные/Закрытые; «План» — **Все/Без плана**/Сегодня/Завтра/Неделя. API: + `?plan=none` (`scheduled_date IS NULL`, просроченные due в него не добавляются) и `?plan=all` (без фильтра); мусор — `400` как и раньше. Тесты: Go `TestPlan_FilterByPlanBuckets` (+ветка `none`), `TestAPI_ListTasksPlan` (`none`/`all` → 200); frontend `tasks-tabs.spec.js` (28e), `DashboardTasksTab.spec.js` (таб-лейбл = выбранный пункт, выбор пункта активирует вкладку, дефолты, «Создать задачу» в обеих), синка e2e (`auth.spec.js`, `projects-to-tasks.spec.js`, `persons.spec.js`, `due-filters-7d.spec.js`). Доки: `docs/api.md` (§ UI 28d/28e, § `?plan=`).
...
### Fixed
- **Тест-флейк «расцветка по due_date» 00:00–03:00 МСК:** `frontend/tests/components/TaskTableDue.spec.js` строил «сегодня» как UTC-дату (`new Date().toISOString().slice(0,10)`), а `dueClassOf` (`src/utils/due-date.js`) считает ЛОКАЛЬНЫЙ день → тест падал каждое утро 00:00–03:00 МСК (обнаружено 2026-09-19 в окне; в CI на UTC не проявляется). Фикс: «сегодня» = `todayKey()` того же модуля — один источник «сегодня» и в компоненте, и в тесте; доказано: старый код падает в МСК-окне, новый — зелёный в МСК и UTC. Тест-баг, поведение UI не менялось.
