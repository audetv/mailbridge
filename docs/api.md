...
- `sort` (v0.25, шаг 7a) — `due` (default) — по сроку возраст, задачи без срока в конце (просроченные выше); `updated` — по дате обновления, новые сверху. Неизвестное значение → `400`.
