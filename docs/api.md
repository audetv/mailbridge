...
- `sort` (v0.25, шаг 7a; **v0.29.0, issue #81** — новый дефолт) — **`created` (default, «По дате создания»)** — по дате создания, новые сверху (`created_at DESC`); `due` — по сроку возраст, задачи без срока в конце (просроченные выше); `updated` — по дате обновления, новые сверху. Неизвестное значение → `400`. **v0.27.1 (хотфикс):** в режиме `due` задачи **без срока** внутри группы сортируются по `created_at DESC` — последние созданные наверху (раньше: `id ASC` — старейшие). Сроки и приоритет просроченных не меняются. **v0.29.0:** UI — «По дате создания» — первый пункт «Сортировки» и дефолт; смена вкладки сбрасывает сортировку на `created`.
...
- `plan` (v0.28.0, шаг 28b) — срез «План»: `today` (scheduled сегодня), `tomorrow` (scheduled завтра), `week` (scheduled в окне 7 дней: сегодня..+6) — **плюс в любом случае просроченные due**: `due_date < сегодня` у активных статусов `new`/`in_progress`/`backlog` (решение владельца 2026-09-19); `completed`/`closed` — вне плана. «Сегодня» — локальный день сервера. **28e:** + `none` (план не установлен, `scheduled_date IS NULL` — вкладка-дропдаун «План», пункт «Без плана»; просрочка в него НЕ добавляется — чистый фильтр по полю плана). **28e:** + `all` — эквивалент «не передан» (без фильтра; пункт «Все» вкладки-дропдауна). Невалидное значение → `400`; не передан — фильтр отключён. Совместим с другими фильтрами (AND).
...
## UI (v0.28.0, шаги 28d/28e — вкладки-дропдауны)

**28e (решение владельца 2026-09-20, аналогия Bootstrap «Tabs with dropdowns»):** «Статус» и «План» — **вкладки-дропдауны**: таб — дропдаун-кнопка, клик открывает меню пунктов, выбор — вкладка становится активной и **таб показывает выбранный пункт** (например «Активные»/«Сегодня» + каретка). **Отдельных селект-фильтров над списком нет** (28d откатился). Пункты: «Статус» — Все/Активные/Бэклог/Выполненные/Закрытые; «План» — Все/Без плана/Сегодня/Завтра/Неделя. Порядок вкладок: **Лента → Статус → План → Проекты → Персоны**; Лента/Проекты/Персоны — обычные вкладки. Задачи живут во вкладках «Статус» и «План» (любой статус, план-срез).

- URL — source of truth (переживает reload): `?tab=status&status=<val>` / `?tab=plan&plan=<val>`.
  - «Статус»: `all` (без фильтра — «Все»), `active`, `backlog`, `completed`, `closed`.
  - «План»: `all` («Все»), `none` («Без плана», `scheduled_date IS NULL`), `today` / `tomorrow` / `week` (API § `?plan=`).
- Дефолты: вкладка «Статус» → «Активные»; вкладка «План» → «Сегодня».
- Неизвестный `?tab=` (в т.ч. старые ключи `?tab=all|active|backlog|completed|closed` — ломаются) → «Статус»/«Все»; невалидный `?status=`/`?plan=` → дефолт вкладки («Все» / «Сегодня»), переписывается в URL.
- `?status=` и `?plan=` — только в своей вкладке; при смене вкладки внешний параметр из URL убирается (`router.replace`).
- Deep-link из «К задачам» (Проекты/Персоны) и ссылки «Проект» в таблице задач — `?tab=status&status=active&…`.
