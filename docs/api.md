...
Вкладки Dashboard: **Лента → Статус → План → Проекты → Персоны**. Задачи живут во вкладках «Статус» и «План», каждая — со своим селект-фильтром над списком:

- **«Статус» + селект** — `?tab=status&status=<val>`: `all` (без фильтра), `active` (`status=new,in_progress`), `backlog`, `completed`, `closed` — те же наборы, что были в старых вкладках.
- **«План» + селект** — `?tab=plan&plan=<val>`: `today` / `tomorrow` / `week` (API § `?plan=`, срез 28b).
- Неизвестный `?tab=` (в т.ч. старые ключи `?tab=all|active|backlog|completed|closed` — ломаются) → «Статус»/«Все»; невалидный `?status=`/`?plan=` → значение сбрасывается на дефолт вкладки («Все» / «Сегодня») и переписывается в URL.
