...
- `:8080` = **ПРОД** mailbridge — НЕ трогать (не kill/restart, не писать в его БД). **Обновление бинарника — только по явной команде владельца**, через `~/apps/mailbridge/deploy-released.sh [тег]` (скачивает релиз из GitHub Releases, health-гейт `/health` + `/api/version`, авто-откат к `bin/mailbridge.bak`). Ручная замена бинарника по-старему запрещена. Журнал: `~/apps/mailbridge/deploy.log`.
...
> **CI на все PR, без исключений (с 2026-09-19, `paths-ignore` в `ci.yml` снят):** — на ЛЮБОМ PR (включая doc-only: `*.md`/`docs/**`/`archive/**`) merge — ТОЛЬКО после `gh pr checks` = pass (CI ~2 мин — фоновое ожидание, не блокирует вёрстку). «no checks reported» — теперь **аномалия** (инцидент: 2026-09-19 PR #76 — ждал ~3 мин проверки, которых не было, владелец «бил ногами»): разобрать (ветка/триггеры), а не мержить вслепую и не ждать бесконечно.
