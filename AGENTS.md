...
1. **Курс:** шапка «Статус» в `PLAN.md` + режим (A разработка / B обсуждение / C баг / hotfix — `CONTRIBUTING.md` §Методология)
2. Ветка от `main`: `feat/stepN-<slug>` (A) / `fix/stepN-<slug>` (C) / `hotfix/<имя>` от последнего тега в прод — **не** `docs/...` (конфликт с папкой `docs/`)
...
> **Модель (2026-09-17):** trunk-based (без `develop`); **шаг = версия** — каждый закрытый шаг = свой минорный тег `v0.x.y` (частые теги — норма); **PATCH = только hotfix** (от последнего тега в прод); тег = quality-гейт и ставится **только по явной команде владельца** после тройки (отчёт + визуал `192.168.1.71:8081` + согласие); агент = PR + merge + доки, **релиз/деплой прод — только владелец**.
